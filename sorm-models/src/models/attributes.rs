...
pub struct StudentWithGranularAttributes {
    ...
    // becuase I have enough info from the annotation to sorm-derive it
    #[sorm(
    ...
    #[sorm(link_many = Book, ty = "array<record<book>>")]
...
pub struct StudentWithDefineAttr {
    ...
    #[sorm(ty = string, define = define_last_name)]
    last_name: String,

    #[sorm(ty = string, define = define_last_name_external_fn_attr)]
    ...
    #[sorm(ty = "int", define = define_age_define_external_fn_path)]
...
pub struct StudentWithDefineFnAttr {
    ...
    #[sorm(ty = int, define = age_define_external_fn_path)]
    age_define_external_fn_path: u8,
...
pub struct Student {
    ...
    // Even if ypu dont list the type for all links, the types are autogenerated at compile time
    // becuase I have enough info from the annotation to sorm-derive it
    #[sorm(link_self = Student, ty = "record<student>")]
    ...
    #[sorm(link_many = Book, ty = "array<record<book>>")]
    // #[sorm(link_many = "Book", ty = "array", item_type = "record(book)")]
    #[serde(rename = "semesterCourses")]
    all_semester_courses: LinkMany<Book>,

    #[sorm(relate(model = StudentWritesBook, connection = "->writes->book"))]
    #[serde(skip_serializing)]
    _written_books: Relate<Book>,

    #[sorm(relate(model = StudentWritesBlog, connection = "->writes->blog"))]
    #[serde(skip_serializing, default)]
    _blogs: Relate<Blog>,
}
...
#[derive(sorm::Edge, Serialize, Deserialize, Debug, Clone, Default)]
#[serde(rename_all = "camelCase")]
#[sorm(table = writes)]
...
// #[sorm-derive(Object, TypedBuilder, Serialize, Deserialize, Debug, Clone, Default)]
// #[serde(rename_all = "camelCase")]
// // #[sorm(table = rocket)]
// pub struct Rocket {
