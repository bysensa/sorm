...
#[tokio::test]
async fn test_creation_with_returning_selected_fields_with_helper_method() -> SurrealOrmResult<()> {
    let db = Surreal::new::<Mem>(()).await.unwrap();
    db.use_ns("test").use_db("test").await.unwrap();
...
async fn test_create_fetch_record_links() -> SurrealOrmResult<()> {
    ...
    let space_ship_names: Option<SpaceShipName> = create()
        .content(unsaved_alien.clone())
...
async fn test_alien_build_output() -> SurrealOrmResult<()> {
    ...
    let build = create().content(unsaved_alien);
...
async fn test_create_set_object_and_array_statement() -> SurrealOrmResult<()> {
    ...
    assert_eq!(selected_spaceships.len(), 2);
    assert_eq!(selected_spaceships[0].name, "SpaceShip2");
    assert_eq!(selected_spaceships[1].name, "SpaceShip1");
    Ok(())
}
